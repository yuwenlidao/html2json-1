...
reCleaner = re.compile(r"(s)?(?P<sep>\W)((?:(?!(?P=sep)).)*)(?P=sep)(?:((?:(?!(?P=sep)).)*)(?P=sep)(g)?)?")

def extract(root, selector, prop, cleaners):
    try:
        if selector == "":
            tag = root
        else:
    ...
    if prop is None:
        v = ''.join(unicode(c) for c in tag.contents if isinstance(c, unicode)).strip()
        if v == "":
            v = tag.text.strip()
    else:
        v = tag[prop].strip()
